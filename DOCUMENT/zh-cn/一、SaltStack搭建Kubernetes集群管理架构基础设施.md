...
``` 
mkdir -p /srv/salt/usr/local/kubelet/
tee /srv/salt/usr/local/kubelet/kubeadm.sls <<-EOF
kubernetes:
  file.managed:
...
```
```
tee /srv/salt/usr/local/kubelet/kubeadm.conf <<-EOF
[Service]
...
```
mkdir -p /srv/salt/usr/local/kubelet
tee /srv/salt/usr/local/kubelet/kubemaster.sls <<-EOF
kubernetes:
  file.managed:
...
```
```
tee /srv/salt/usr/local/kubelet/kubemaster.sh <<-EOF
images=(
...
salt '*' cmd.run 'mkdir -p /usr/local/kubelet'
...
#salt '*' cmd.run 'chmod +x /usr/local/kubelet/kubemaster.sh && /usr/local/kubelet/kubemaster.sh'
chmod +x /usr/local/kubelet/kubemaster.sh && /usr/local/kubelet/kubemaster.sh
```
